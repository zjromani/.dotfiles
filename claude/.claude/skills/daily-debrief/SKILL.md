...
Spawn one `general-purpose` subagent with the resolved date scope from Step 1 (today's date, or the Friday–Monday range on a Monday) and this exact job — it has no access to this file, so pass the scope and calls literally:
